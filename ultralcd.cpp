...
static void lcd_prepare_menu()
{
    ...
    MENU_ITEM(submenu, MSG_SET_ORIGIN, lcd_set_axis_zero);
#ifdef SDSUPPORT
...
static void lcd_set_axis_zero()
{
    START_MENU();
    MENU_ITEM(back, MSG_PREPARE, lcd_prepare_menu);
    MENU_ITEM(gcode, MSG_SET_X_ZERO, PSTR("G92 X0"));
    MENU_ITEM(gcode, MSG_SET_Y_ZERO, PSTR("G92 Y0"));
